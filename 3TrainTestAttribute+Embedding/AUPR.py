...
while counter0 < 5:
    ...
    precision, recall, thresholds = precision_recall_curve(Real, PredictionProb)
    ...
    #         label='fold %d (AUPR = %0.4f)' % (i, roc_auc), visible = False)
    #plt.plot(label = 'fold %d (AUC = %0.4f)' % (i,roc_auc), color = colorlist[i])
...
storFile(MyNew, '5aupr折的表（由保存值生成）svm.csv')
print(MyNew)
...
while counter < len(mean_fpr):
    ...
    counter = counter + 1
storFile(FprAndTpr, 'RecallAndPrecisionAttributeMannersvm.csv')


plt.plot(mean_fpr, mean_tpr, color='black',
         label=r'Mean (AUPR = %0.4f)' % (mean_auc),
         lw=2, alpha=1)
...
plt.legend(bbox_to_anchor=(0.4, 0.6), fontsize='small')
plt.plot([0, 1], [1, 0], color='navy', lw=2, linestyle='--')
# plt.grid(linestyle='-')
# # 保存图片
plt.savefig('ROC-5foldAUPRsvm.svg')
plt.savefig('ROC-5foldAUPRsvm.tif')
plt.show()
